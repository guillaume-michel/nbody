...
nbody_cpp: nbody.cpp
	clang++-12 -pipe -Ofast -fomit-frame-pointer -march=$(arch) -mllvm -unroll-threshold=500 $^ -o $@
...
clean:
	rm -rf nbody_rust nbody_cpp nbody_c nbody_lisp nbody.fasl

bench: nbody_rust nbody_cpp nbody_c nbody_lisp
	@echo "-----------RUST----------------"
	@time ./nbody_rust $(N)
