...
public:
  NBodySystem()
      : bodies{{Body::sun(), Body::jupiter(), Body::saturn(), Body::uranus(),
                Body::neptune()}} {
    double px = 0.0;
    double py = 0.0;
    double pz = 0.0;
  ...
  void advance(double dt) {
    ...
    for (unsigned i = 0; i < N; i += 2) {
      ...
      __m128d dSquared =
          _mm_add_pd(_mm_add_pd(_mm_mul_pd(dx, dx), _mm_mul_pd(dy, dy)),
                     _mm_mul_pd(dz, dz));
      ...
      __m128d dmag =
          _mm_mul_pd(_mm_div_pd(_mm_set1_pd(dt), dSquared), distance);
